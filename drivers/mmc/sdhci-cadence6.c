...
/*
 * NOTE: DLL reset is required before and after updating the PHY_DLL_SLAVE_CTRL_REG_ADDR register.
 * Without toggling the DLL reset, tuning was consistently failing in SDR104 mode.
 * This requirement is not documented in the user guide, but observed during debugging and testing.
 * The sequence below ensures reliable tuning by switching on the DLL reset before the register
 * update, and switching it off immediately after.
 */
	sdhci_cdns6_reset_phy_dll(plat, true);
